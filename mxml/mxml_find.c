...
static int is_key_self_or_child(const char *child, int childlen, const char *parent, int parentlen) {
 ...
 **/
static const char* find_key_noedit(struct mxml *m, const char *reqkey, int reqkeylen, size_t *sz_return) {
    const char *dot;
    ...
    if (dot) {
        parent_text = find_key_noedit(m, reqkey, dot - reqkey, &parent_sz);
        if (!parent_text)
 ...
 */
const char* find_key(struct mxml *m, const char *reqkey, int reqkeylen, size_t *sz_return) {
    const struct edit *e;
    ...
    for (e = m->edits; e; e = e->next) {
        switch (e->op) {
            case EDIT_DELETE:
                if (is_key_self_or_child(reqkey, reqkeylen, e->key, strlen(e->key))) {
                    errno = ENOENT;
                    return NULL;
                }
                break;
            case EDIT_APPEND:
                ...
                if (is_key_self_or_child(e->key, strlen(e->key), reqkey, reqkeylen))
                    implied_by_append = 1;
                // no break
            case EDIT_SET:
                if (strncmp(e->key, reqkey, reqkeylen) == 0 && e->key[reqkeylen] == '\0') {
