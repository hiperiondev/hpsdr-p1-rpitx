 ...
 */
static size_t unencode_xml_into(const struct cursor *init_curs, char *out) {
    size_t n = 0;
...
int mxml_delete(struct mxml *m, const char *key) {
    ...
    if (ends_with(key, "[$]")) {
        ...
        if (content && !total) {
            ...
            edit = edit_new(m, EDIT_SET, ekey, ekeylen, total);
            if (!edit)
                return -1;
        } else if (!content && total) {
            /* XXX this should never happen */
            edit = edit_new(m, EDIT_APPEND, ekey, ekeylen, total);
            if (!edit)
                return -1;
        }
    }
