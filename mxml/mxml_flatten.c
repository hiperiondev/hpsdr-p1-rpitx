...
static const char* last_tag(const char *key) {
...
static unsigned int parent_len(const char *key, unsigned int keylen) {
    ...
    for (edit = m->edits; edit; edit = edit->next, es++) {
        ...
        switch (edit->op) {
            ...
            case EDIT_SET:
                ...
                strncpy(es->set.token.key, edit->key, sizeof es->set.token.key);
            ...
            case EDIT_APPEND:
                ...
                es->append.parentlen = parent_len(es->append.key, es->append.keylen);
                es->append.value = edit->value;
                es->append.valuelen = -1;
                if (snprintf(es->append.tagdata, sizeof es->append.tagdata, "</%s>", last_tag(edit->key)) >= sizeof es->append.tagdata)
                    goto nomem;
                break;
        }
    }
 ...
 */
static int xml_tokenize(struct xmlstate *x) {
    const char *tag;
    int taglen;
 ...
 */
static int process_set(struct setstate *set, struct token **carrier) {
    struct token *token = *carrier;

    if (token && set->token.keylen == token->keylen && memcmp(token->key, set->token.key, token->keylen) == 0) {
        if (token->type == TOK_OPEN)
            set->just_opened = 1;
 ...
 */
static int process_append(struct appendstate *app, struct token **carrier) {
    struct token *token = *carrier;

    /* Is this the parental CLOSE token that triggers us? */
    if (token && token->type == TOK_CLOSE && app->parentlen == token->keylen && memcmp(app->key, token->key, token->keylen) == 0) {
...
static int process_token(struct editstate *s, struct token **carrier) {
    struct token *token = *carrier;
    ...
    switch (s->kind) {
        ...
        case EDIT_KIND_DELETE:
            if (token && s->del.keylen <= token->keylen && (s->del.keylen == token->keylen || token->key[s->del.keylen] == '.')
